...
import { FormularioDeCompra } from "./formulario";
...
export default async function ConfirmarCompraPage() {
  const carrito = await obtenerCarrito();

  // Sin nada en el presupuesto no hay nada que confirmar.
  // Un carrito con solo cortes no está vacío: alguien puede venir del
  // armador de cortes sin haber agregado ningún producto.
  if (carrito.items.length === 0 && carrito.cortes.length === 0) {
    redirect("/carrito");
  }
  ...
  return (
    <div className="min-h-screen bg-sitio-alt">
      {/* Sin banda oscura, igual que el presupuesto: el carrito es una sola
      ...
      <div className="contenedor space-y-5 pb-[70px] pt-6">
        ...
        {(conPrecio.length > 0 || carrito.cortes.length > 0) && (
        <FormularioDeCompra
          ...
          //
          // Los cortes van como un renglón cada uno: son un trabajo, no un
          // producto con cantidad, y lo que hay que poder leer es de qué placa
          // salen y cuántas piezas tienen.
          items={[
            ...conPrecio.map((i) => ({
              id: i.id,
              descripcion: i.descripcion,
            ...
            ...carrito.cortes.map((c) => ({
              id: c.id,
              descripcion: `Corte a medida — ${c.material} (${c.piezas} ${
