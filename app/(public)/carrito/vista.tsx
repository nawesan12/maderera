...
import { quitarCorteDelCarrito } from "@/app/(public)/corte/actions";
import type { CorteEnCarrito } from "@/lib/dal/carrito";

/**
 * La pantalla del carrito.
 *
 * Se llamaba «presupuesto» y la barra de arriba decía «Carrito»: dos nombres
 * para lo mismo, y el que la gente usa es el segundo. El pedido de presupuesto
 * por escrito sigue estando, pero es una de las tres salidas del carrito, no el
...
export function VistaDelCarrito({
  ...
  /**
   * Los cortes a medida que hay en el carrito.
   *
   * Vienen del servidor y no del contexto: no tienen cantidad que subir ni
   * bajar —un despiece es lo que es— así que no hay nada que actualizar de
   * forma optimista. Quitar uno recarga la página.
   */
  ...
  const [quitandoCorte, setQuitandoCorte] = useState<string | null>(null);
  const router = useRouter();

  // El contexto solo conoce los productos: los cortes se suman acá.
  const subtotalCortes = cortes.reduce((total, c) => total + c.total, 0);
  const totalGeneral = subtotal + subtotalCortes;
  ...
  if (items.length === 0 && cortes.length === 0) {
    return (
      ...
      <div className="contenedor pb-[70px] pt-6">
        <div className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_344px] lg:items-start">
          ...
          <div>
            ...
            {/* Los cortes a medida, arriba de los productos.

                Van en su propia tarjeta porque no son un producto con cantidad:
                cada uno es una placa, un despiece y un acomodo, y lo que hay
                que poder leer de un vistazo es de qué placa sale y cuántas
                piezas tiene. Se quitan enteros; para cambiar una medida se
                arma de nuevo, que es más rápido que editar renglones. */}
            {cortes.length > 0 && (
                ...
                <CardContent className="p-0">
                  <ul className="divide-y divide-linea-tenue">
                    {cortes.map((corte) => (
                      ...
                      >
                        <span className="flex h-11 w-11 shrink-0 items-center justify-center rounded-[10px] bg-naranja-claro text-acento-sobre-claro">
                          <Scissors className="h-5 w-5" />
                        </span>

                        <div className="min-w-0 flex-1">
                          <p className="text-[15.5px] font-semibold leading-snug">
                            Corte a medida — {corte.material}
                          </p>
                          <p className="mt-0.5 text-sm text-texto-2">
                            {corte.mitad
                              ? `Media placa de ${corte.placaLargoMm} × ${corte.placaAnchoMm} mm`
                              : `Placa de ${corte.placaLargoMm} × ${corte.placaAnchoMm} mm`}
                            {" · "}
                            {corte.piezas}{" "}
                            {corte.piezas === 1 ? "pieza" : "piezas"}
                            {corte.pasadas > 0 &&
                              ` · ${corte.pasadas} ${corte.pasadas === 1 ? "pasada" : "pasadas"}`}
                            {corte.cantoDescripcion &&
                              ` · canto ${corte.cantoDescripcion}`}
                          </p>
                        </div>

                        <p className="tabular w-[110px] text-right text-base font-semibold">
                          {formatearPrecio(String(corte.total))}
                        </p>

                        <button
                          onClick={() => {
                            setQuitandoCorte(corte.id);
                            quitarCorteDelCarrito(corte.id).then(() => {
                              setQuitandoCorte(null);
                              router.refresh();
                            });
                          }}
                          disabled={quitandoCorte === corte.id}
                          ...
                          aria-label={`Sacar el corte de ${corte.material} del carrito`}
                        >
                    ...
                    >
                      <div className="min-w-48 flex-1">
                        ...
                        {/* Que se cotiza a mano se dice acá, en el renglón, y
                            no recién en el carrito: es lo que explica por qué
                      ...
                      <button
                        ...
                        disabled={guardando}
                        className="flex h-[38px] w-[38px] shrink-0 items-center justify-center rounded-[9px] border border-linea-suave text-texto-3 transition-colors hover:text-rojo-oferta disabled:opacity-50"
                        aria-label={`Sacar ${item.descripcion} del carrito`}
                      >
                        <Trash2 className="h-4 w-4" />
                      </button>
                    </li>
              ...
              <CardContent className="p-5">
                  ...
                  {cortes.length > 0 && (
                    <div className="flex justify-between gap-3">
                      <dt className="text-texto-2">
                        {cortes.length === 1
                          ? "1 corte a medida"
                          : `${cortes.length} cortes a medida`}
                      </dt>
                      <dd className="tabular font-semibold">
                        {formatearPrecio(String(subtotalCortes))}
                      </dd>
                    </div>
                  )}
                  <div className="flex justify-between gap-3">
                ...
                <div className="mt-3 flex items-baseline justify-between gap-3 border-t border-linea-suave pt-3">
                  ...
                  <span className="tabular text-[22px] font-bold tracking-[-0.02em]">
                    {formatearPrecio(String(totalGeneral))}
                  </span>
                </div>

                {/* Ley 27.743: el neto también acá, que es donde se mira el
                    número que se va a pagar. */}
                <PrecioSecundario precioFinal={totalGeneral} className="mt-2" />

                <div className="mt-4 space-y-2.5">
                  <Button
                    render={<Link href="/carrito/confirmar" />}
            ...
            {!esProfesional && (
              <section className="rounded-xl border border-[#f5d9b8] bg-naranja-claro px-[18px] py-4 dark:border-brand-orange/25">
                ...
                <p className="mt-1.5 text-[13.5px] leading-normal text-acento-sobre-claro">
                  Con cuenta habilitada este carrito sale con tu escala de
                  descuento aplicada.
                </p>
              </section>
            )}
 ...
 * catálogo es un producto elegido: cuando hay que revisar el carrito,
