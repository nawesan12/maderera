...
import { Truck, MapPin, Clock, Package, Check, ArrowRight } from "lucide-react";
import { Card, CardContent } from "@/components/ui/card";
import { Badge } from "@/components/ui/badge";
import { Button } from "@/components/ui/button";
import { pedidos } from "@/lib/dashboard-data";
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-4">
        {[
          { label: "Preparando", value: pedidos.filter((p) => p.estado === "preparando").length, color: "text-yellow-400" },
          { label: "Listos", value: pedidos.filter((p) => p.estado === "listo").length, color: "text-blue-400" },
          { label: "En Camino", value: pedidos.filter((p) => p.estado === "en-camino").length, color: "text-brand-orange" },
          { label: "Entregados Hoy", value: pedidos.filter((p) => p.estado === "entregado").length, color: "text-green-400" },
        ].map((s) => (
      ...
      <div className="space-y-3">
        {pedidos.map((pedido) => {
          ...
          return (
            <Card key={pedido.id} className="bg-white/[0.03] border-white/5 hover:bg-white/[0.05] transition-colors">
              <CardContent className="p-5">
                ...
                <div className="flex items-center gap-1 mt-4 ml-14">
                  {["preparando", "listo", pedido.tipo === "entrega" ? "en-camino" : null, "entregado"].filter(Boolean).map((step, i, arr) => {
                    const estados = ["preparando", "listo", "en-camino", "entregado"];
                    const currentIdx = estados.indexOf(pedido.estado);
                    const stepIdx = estados.indexOf(step!);
                    const isDone = stepIdx <= currentIdx;
                    return (
                      <div key={step} className="flex items-center gap-1 flex-1">
                        <div className={`w-2 h-2 rounded-full shrink-0 ${isDone ? "bg-brand-orange" : "bg-white/10"}`} />
                        {i < arr.length - 1 && <div className={`h-0.5 flex-1 rounded ${isDone ? "bg-brand-orange" : "bg-white/10"}`} />}
                      </div>
                    );
                  })}
                </div>
